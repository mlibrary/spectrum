...
RSpec.describe "tools/list" do
  ...
  it "lists the four spectrum tools" do
    names = tools_list["result"]["tools"].map { |tool| tool["name"] }
    expect(names).to contain_exactly("spectrum_search", "spectrum_get_record", "spectrum_export_ris", "spectrum_list_filters")
  end
  ...
  it "describes the list_filters tool's required arguments" do
    tool = tools_list["result"]["tools"].find { |t| t["name"] == "spectrum_list_filters" }
    expect(tool["inputSchema"]["required"]).to eq(["datastore"])
  end
end
